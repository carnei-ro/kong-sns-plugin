...
config:
	...
	@curl -s -X POST http://localhost:8001/services/sns/plugins --data "name=${NAME}" --data-urlencode "config.aws_iam_role=my-iam-role" --data "config.aws_region=sa-east-1" --data "config.queue_name=MY_TOPIC" --data "config.aws_account_id=123456789012"
