...
function plugin:access(conf)
  ...
  local message_body=ngx.escape_uri(cjson.encode(body))
  local host = fmt("sns.%s.amazonaws.com", conf.aws_region)
  local path = "/"
  local topic_arn = concat({"arn%3Aaws%3Asns%3A", conf.aws_region, "%3A", conf.aws_account_id,"%3A",conf.topic_name})
  local query = concat({'Version=2010-03-31&Action=Publish&TopicArn=' , topic_arn , '&Message=' , message_body})
